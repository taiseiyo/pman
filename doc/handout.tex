...
\section{概要}
\label{sec:org62d6f74}
\begin{frame}[label={sec:orge5ac36d}]{Python とは？}
...
本日のソース一式
→ git clone \url{https://github.com/taiseiyo/pman.git}
\end{frame}

\section{Python 環境作成}
\label{sec:org17e54c9}
\begin{frame}[label={sec:org2364cbc},fragile]{Python 導入：apt install を使う(Debian)}
 メリット
\begin{itemize}
\item インストールが簡単
\end{itemize}
...
\begin{frame}[label={sec:org1002e9e},fragile]{Python 導入：source からビルド}
 メリット
\begin{itemize}
\item 最新バージョンが手に入る
\end{itemize}
...
\label{sec:org75031ef}
\begin{frame}[label={sec:orge536224}]{pman コマンド}
発想
\begin{itemize}
...
\section{実践(ハンズオン)}
\label{sec:org97355ac}
\begin{frame}[label={sec:orgb2bb603},fragile]{実践：必要なモジュール}
 \begin{itemize}
\item beautifulsoup4
\begin{itemize}
...
\begin{frame}[label={sec:org97d3ce1}]{実践：プログラム作成}
HTML 取得
\begin{itemize}
...
\section{最後に}
\label{sec:org6c7502d}
\begin{frame}[label={sec:org715f59d}]{最後に}
GitHub: 
\end{frame}
\end{document}