...
\section{概要}
\label{sec:org349cfae}
\begin{frame}[label={sec:org9a827b5}]{Python とは？}
...
\section{Python 環境作成}
\label{sec:org9eecb33}
\begin{frame}[label={sec:org1da7f8c},fragile]{Python 導入：apt install を使う(Debian)}
 メリット
\begin{itemize}
\item インストールが簡単
\end{itemize}
...
\begin{frame}[label={sec:orgce78655},fragile]{Python 導入：source からビルド}
 メリット
\begin{itemize}
\item 最新バージョンが手に入る
\end{itemize}
...
\label{sec:orgd56a749}
\begin{frame}[label={sec:org14457dd}]{pman コマンド}
発想
\begin{itemize}
...
\section{実践(ハンズオン)}
\label{sec:orgb10f7d9}
\begin{frame}[label={sec:org2a7e2f6},fragile]{実践：必要なモジュール}
 \begin{itemize}
\item beautifulsoup4
\begin{itemize}
...
\begin{frame}[label={sec:orgdf18289}]{実践：プログラム作成}
HTML 取得
\begin{itemize}
...
\section{最後に}
\label{sec:org1093c30}
\begin{frame}[label={sec:org3a72277}]{最後に}
GitHub: 
\end{frame}
\end{document}